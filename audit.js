...
console.log(`\n  ΣΥΝΟΛΟ ΒΑΣΗΣ: ${grandTotal} ερωτήσεις`);
console.log(`  Στενό σημείο (no-repeat games): ${minInfo} = ${minTier} παιχνίδια χωρίς επανάληψη`);

/* ---------- 2. Στόχος 30/tier ---------- */
console.log('\n🎯 ΣΤΟΧΟΣ 30 ΜΟΝΑΔΙΚΩΝ ΠΑΙΧΝΙΔΙΩΝ\n');
const allOk = rows.every(r => r.easy >= TARGET_PER_TIER && r.medium >= TARGET_PER_TIER && r.hard >= TARGET_PER_TIER);
if(allOk){
  console.log('  ✅ Όλες οι κατηγορίες, όλα τα tiers ≥30. Ο στόχος έχει επιτευχθεί πλήρως!');
} else {
  console.log('  ⚠️  Δεν έχουν όλα τα tiers φτάσει τα 30 ακόμα — δες τις προειδοποιήσεις παραπάνω.');
}
